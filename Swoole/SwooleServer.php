...
class SwooleServer {
    ...
    public function __construct($host = '0.0.0.0', $port = 9501,$daemon=1) {
    ...
    public function run() {
        ...
        $this->_serv->set(array(
            ...
            //'heartbeat_check_interval' => 30,   //每--秒侦测一次心跳
            'heartbeat_idle_time' => 60,        //如果在--秒内未向服务器端发送数据，将会被切断
        ));
    ...
    public function onWorkerStart($serv, $workerId) {
        ...
        // $result['cmd']    = "server_start";
        // $ClientService = \Common\Service\ClientService::instance();
        // $ret = $ClientService->client_construct($serv,$result);
    }
    ...
    public function onReceive($serv, $fd, $from_id, $data) {
        ...
        // error_log(date('Y-m-d H:i:s').' | onReceive:'.trim($data)."\n",3,SWOOLE_PATH . DIRECTORY_SEPARATOR .'Public/onReceive.log');
        ...
        switch ($result['action']) {
            case 'reload':  //重启
                ...
                break;
            case 'list':  //客户端列表
                $serv->send($fd, json_encode($serv->getClientList($start_fd)));
                break;
            case 'close':  //关闭
                $serv->shutdown();
                break;
